DO $$
    BEGIN
        IF EXISTS (SELECT 1 FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = 'nertz' AND TABLE_NAME = 'games') THEN
            IF NOT EXISTS (SELECT 1 FROM INFORMATION_SCHEMA.TABLE_CONSTRAINTS WHERE TABLE_SCHEMA = 'nertz' AND CONSTRAINT_NAME = 'fk_games_rooms' AND CONSTRAINT_TYPE = 'FOREIGN KEY') THEN
                ALTER TABLE nertz.games
                    ADD CONSTRAINT fk_games_rooms FOREIGN KEY (room_id)
                        REFERENCES nertz.rooms(id);
            END IF;
        END IF;
    END $$;
DO $$
